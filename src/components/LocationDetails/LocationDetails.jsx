...
const LocationDetails = ({ locationId, location_details, visited_locations, onModalLocationsChanged }) => {
  ...
  return (
    <>
      <div className="absolute top-20 left-10 h-4/5 w-96 max-w-sm bg-white shadow-lg rounded-lg overflow-hidden my-4">
        <img className="w-full h-96 object-cover object-center" src="https://www.locationscout.net/build/images/home/hallerbos.jpg" alt="avatar" />
        <div className="py-4 px-6">
          <h1 className="text-2xl font-semibold text-gray-800">{location_details.name}</h1>
          <p className="py-2 text-lg text-gray-700">{location_details.description}</p>
          ...
          <div className="flex items-center mt-4 text-gray-700">
            ...
            <h1 className="px-2 text-sm" onClick={
              () => {
                updateLocations(locationId);
                isChecked();
              }
            }>{check ? "Check" : "Uncheck"}</h1>
          </div>
        </div>
      </div>
    </>
