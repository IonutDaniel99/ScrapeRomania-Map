...
const Maps = () => {
   ...
   return (
      <>
               ...
               <>
                  ...
                  <svg xmlns='http://www.w3.org/2000/svg' version='1.1' baseProfile='tiny' width={1200} height={800}>
                     {Object.values(locationsList).map((location, i) => {
                        ...
                        return <SvgDisplay svg_ref={svg_ref} imageFill={imageFill} isPanMoving={isPanMoving} key={locationId} location={location} locationId={locationId} locationModalLogic={(id) => locationModalLogic(id)} setLocationDetails={(location) => setLocationDetails(location)} />
                     })}
                  </svg>
               </>
            )}
         ...
         {
            <Transition show={locationDetailsIsOpen} enter='transition-opacity duration-1000' enterFrom='opacity-0' enterTo='opacity-100' leave='transition-opacity duration-1000' leaveFrom='opacity-100' leaveTo='opacity-0'>
               <LocationDetailsModal key={locationId} locationId={locationId} location_details={locationDetails} visited_locations={visitedLocations} onModalLocationsChanged={onModalLocationsChanged} onHandleChildCloseModal={onHandleChildCloseModal} />
            </Transition>
         }
      </>
   )
