...
const Maps = () => {
  ...
  const [displayPanZoomDisplayNumbers, setDisplayPanZoomDisplayNumbers] =
    useState(false);
  ...
  return (
    <>
      {isLoading && (
        <div
          className={
            "absolute top-0 left-0 w-full h-full z-50 flex justify-center items-center"
          }
        >
          <ReactLoading
            type={"bubbles"}
      ...
      <div className="relative" ref={gPanZoomRef}>
        {!isLoading && (
          <>
            <div
              id="locations_numbers"
              ref={locationsNumberRef}
              className={"z-50"}
            >
                ...
                if (svg_ref.current != null) {
                  return (
                    <div
                      ...
                      id={`location_svg_${i}`}
                      className={
                        "absolute top-0 left-0 text-[6px] ml-1 mt-[10px] font-semibold"
                      }
                      style={{
                        transform: `translate(${location.number_coords[0]}px,${location.number_coords[1]}px)`,
                        visibility: displayPanZoomDisplayNumbers
                          ? "visible"
                          : "hidden",
                      }}
                    >
